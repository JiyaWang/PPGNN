...
def load_nc_dataset(args):
    ...
    if dataname == 'deezer-europe':
        ...
        dataset = load_ogb_dataset(args.data_dir, dataname)
    else:
...
def load_planetoid_dataset(name, no_feat_norm=False):
    ...
    if not no_feat_norm:
        ...
        torch_dataset = Planetoid(root=f'{DATAPATH}', name=name, transform=transform)
    else:
        torch_dataset = Planetoid(root=f'{DATAPATH}', name=name)
    data = torch_dataset[0] 

    edge_index = data.edge_index #边
    node_feat = data.x #
    label = data.y
    ...
    dataset.train_idx = np.arange(data.num_nodes -1000)
    dataset.valid_idx = np.arange(data.num_nodes -1000, data.num_nodes - 500)
    dataset.test_idx = np.arange(data.num_nodes - 500, data.num_nodes)
...
def load_amazon_dataset(name, no_feat_norm=False):
    ...
    if not no_feat_norm:
        transform = T.NormalizeFeatures()
        torch_dataset = Amazon(root=f'{DATAPATH}', name=name, transform=transform)
    else:
        torch_dataset = Amazon(root=f'{DATAPATH}', name=name)
    data = torch_dataset[0] 
